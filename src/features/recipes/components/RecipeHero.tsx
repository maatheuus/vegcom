...
export function RecipeHero({ className, hightlightedRecipe }: RecipeHeroProps) {
  ...
  return (
    <Link href={`/recipes/${slug}`} className={clsx("contents", className)}>
      ...
      <div className="group relative overflow-hidden rounded-xl shadow-md transition-all duration-500 hover:shadow-lg">
        <div className="relative aspect-[16/9] sm:aspect-[21/9]">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent opacity-0 transition-opacity duration-500 ease-out group-hover:opacity-100" />
        </div>
        ...
        <div className="absolute inset-0 flex w-full flex-col justify-end gap-y-4 p-4 sm:p-6 md:p-8">
          ...
          <div className="mt-3 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="font-maitree flex translate-y-6 flex-wrap items-center gap-x-2 font-semibold text-green-50 opacity-0 transition-all delay-100 duration-500 ease-out group-hover:translate-y-0 group-hover:opacity-100 sm:gap-x-6">
              {rating && (
            ...
            <Button
              size="lg"
              className="font-lora w-full translate-y-6 cursor-pointer rounded-full bg-green-50 px-6 py-2.5 text-sm text-green-500 italic opacity-0 transition-all delay-200 duration-500 ease-out group-hover:translate-y-0 group-hover:scale-105 group-hover:opacity-100 sm:w-auto sm:text-base"
            >
              Ver receita
            </Button>
          </div>
