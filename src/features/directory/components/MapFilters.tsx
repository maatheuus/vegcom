...
function FilterButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`inline-flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-xs font-bold transition ${active ? "bg-green-500 text-white shadow-sm" : "text-green-500 hover:bg-green-100"}`}
    >
      {icon}
      {label}
      <span
...
function CategoryButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-bold transition ${active ? "bg-green-500 text-white" : "text-green-500 hover:bg-green-100"}`}
    >
      {children}
    </button>
  );
